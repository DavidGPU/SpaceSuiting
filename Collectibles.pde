...
class HealthCollectibleForSale extends GameObject {
  ...
  void update() {
    for (SpawnManager sm : spawnManagers) { 
      for (Spaceship s : spaceships) {
        if (isCollidingWith(s) && vKeyPressed && money >= 100 * (sm.stage * sm.stage)) {
          collected = true;
          money -= 100 * (sm.stage * sm.stage);
        }
      }
    }
    super.update();
...
class SuperDamageCollectibleForSale extends GameObject {
  ...
  void update() {
    for (SpawnManager sm : spawnManagers) { 
      for (Spaceship s : spaceships) {
        if (isCollidingWith(s) && vKeyPressed && money >= 75 * (sm.stage * sm.stage)) {
          collected = true;
          money -= 75 * (sm.stage * sm.stage);
        }
      }
    }
